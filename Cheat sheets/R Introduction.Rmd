...
# The c() function creates a vector, in which all elements are the same type. In the first case, the elements are numeric, in the second, they are characters.
```
...
# object1 <- read.csv("Mispriced-Diamonds.csv")

# Import a local tab delimited file
# object2 <- read.table("string.txt", sep = "\t")

# Import ana local csv file from a different working directory
# object3 <- read.csv("C:/Desktop/OfficeSupplies.csv")

# Browse and select the local file
# object4 <- read.csv(file.choose())

# Downloading Remote Data by accessing files from the web
# object6 <- read.csv("http://sumsar.net/files/posts/2014-02-04-bayesian-first-aid-one-sample-t-test/roubik_2002_coffe_yield.csv")
...
Select rows from 3rd to 15th except 4, 5 and 6th rows, bring only 2nd and 3rd column
```{r}
